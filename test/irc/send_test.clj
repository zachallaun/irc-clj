...
(facts "construct-message :err-need-more-params"
  (construct-message server user1 {:message :err-need-more-params :command :join})
  => ":localhost 461 user1 JOIN :Not enough parameters"

  (construct-message server user1 {:message :err-need-more-params :command :user})
  => ":localhost 461 user1 USER :Not enough parameters"

  (construct-message server user1 {:message :err-need-more-params :command :part})
...
(facts "construct-message :nick"
  (construct-message server user1 {:message :nick :source "user3" :nick "user99"})
  => ":user3 NICK user99"
  )

(facts "construct-message :join"
  (construct-message server user1 {:message :join :source "user3" :chan "#chan3"})
  => ":user3 JOIN #chan3"
  )

(facts "construct-message :part"
  (construct-message server user1 {:message :part :source "user3" :chan "#chan3"})
  => ":user3 PART #chan3"
  )
...
(facts "construct-message :quit"
  (construct-message server user1 {:message :quit :source "user3" :text "Client Quit"})
