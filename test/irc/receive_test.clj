...
(facts "process-command :privmsg"
  ;; user not registered
  (drain-all!)
  (process-command server-none uid {:command :privmsg :target "#chan1" :text "hello world"})
  => (validate-state server-none
                     (merge {user [(str ":localhost 451 * "
                                        ":You have not registered")]}
                            (no-messages [user1 user2 user3 user4])))
  ...
  ;; non-existent target
  (drain-all!)
  (process-command server uid1 {:command :privmsg :target "#chan99" :text "hello world"})
  => (validate-state server
...
(facts ":process-command quit"
  ;; quit
  (drain-all!)
  (process-command server uid1 {:command :quit :source "user1" :text "Client Quit"})
