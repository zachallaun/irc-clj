...
(defn add-user-to-channel [server uid cname]
  (-> server
      (assoc-in [:channels cname] (channel-add-user (channel-by-name server cname) uid))
      (assoc-in [:users uid] (user-add-channel (user-by-uid server uid) cname))))

(defn remove-user-from-channel [server uid cname]
  (-> server
      (assoc-in [:channels cname] (channel-remove-user (channel-by-name server cname) uid))
      (assoc-in [:users uid] (user-remove-channel (user-by-uid server uid) cname))))
